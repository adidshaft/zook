...
import { Clock3, Printer, QrCode, RefreshCcw, ShieldCheck, Users } from "lucide-react";
...
function QueueStatusMark({ label, flagged = false }: { label: string; flagged?: boolean }) {
  return (
    <span
      aria-label={label}
      title={label}
...
export function AttendanceQrPanel({
  ...
  return (
    <GlassCard variant="strong" className="attendance-qr-panel rounded-[24px] border-[var(--border)] bg-[var(--bg-sunken)] shadow-none">
            ...
            <div className="flex min-w-0 flex-col justify-between gap-4">
              <div>
                <p className="text-sm leading-6 text-[var(--text-secondary)]">
                  {isStatic
                    ? "Static QR — print and display at the entry gate."
                    : "Show this QR at reception. It refreshes every 30 seconds."}
                </p>
              ...
              <details className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-sunken)]/60 px-4 py-3">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-sm font-medium text-[var(--text-secondary)]">
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-[var(--text-secondary)]">
                  {[
                    ...
                    ["Branch check", "Restricted to the selected branch"],
                    ["Membership check", "Active plan & status verified"],
                  ].map(([item, detail]) => (
                    <span
                      key={item}
                      title={detail}
                      className="rounded-full border border-[var(--border-subtle)] bg-[var(--surface-raised)] px-2 py-1 text-[var(--text-secondary)]"
                    >
                      {item}
                    </span>
                  ))}
      ...
      <div className="mt-5 rounded-[20px] border border-[var(--border-subtle)] bg-[var(--bg-sunken)]/40 px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          ...
          {(queueRecords.length ? queueRecords.slice(0, 4) : []).map((record) => {
            ...
            return (
              <div key={record.id} className="flex items-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-raised)] px-3 py-2">
                <QueueStatusMark label={flagged ? "Flagged" : "Pending"} flagged={flagged} />
