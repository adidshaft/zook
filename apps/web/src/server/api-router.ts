...
async function handleAuth(request: NextRequest, path: string[]) {
  ...
  if (request.method === "POST" && pathMatches(path, ["auth", "logout"])) {
    ...
    response.cookies.delete(refreshSessionCookieName);
    return response;
  }
  if (request.method === "POST" && pathMatches(path, ["auth", "refresh"])) {
    ...
    if (!refreshToken) {
      return fail("UNAUTHORIZED", "Refresh token required", 401);
    }
    const now = new Date();
    ...
    if (!currentSession) {
      return fail("UNAUTHORIZED", "Refresh token expired", 401);
    }
    const token = AuthService.createToken();
    const expiresAt = new Date(Date.now() + 15 * 60 * 1000);
...
async function handleMeData(request: NextRequest, path: string[]) {
  ...
  if (request.method === "PATCH" && pathMatches(path, ["me", "profile"])) {
    ...
    const [user, profile, latestBodyProgress] = await prisma.$transaction(async (tx) => {
      ...
      const nextIsMinor = minorFromDate ?? currentUser.isMinor;
        ...
        data: clean({
          ...
          ...(minorFromDate !== undefined
            ? {
                ...
                guardianPending: minorFromDate ? true : false,
              }
            : {}),
          gender: body.gender,
          emergencyContact:
          ...
          marketingOptIn: nextIsMinor ? false : body.marketingOptIn,
          aiConsent: nextIsMinor ? false : body.aiConsent,
...
async function handleOrganizations(request: NextRequest, path: string[]) {
  ...
  if (request.method === "POST" && pathMatches(path, ["orgs", /.+/, "billing", "mandate"])) {
    ...
    const checkoutUrl = createdMandate.checkoutUrl ?? `/checkout/${session.id}`;
