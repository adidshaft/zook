...
let activeRegularFont: FontName = FONT_REGULAR;
let activeBoldFont: FontName = FONT_BOLD;

type Point = { x: number; y: number };
type PageMap = Record<"cover" | "master" | "ios" | "android", PageNode>;
...
function makeSolidPaint(hex: string, opacity = 1): SolidPaint {
  return {
    type: "SOLID",
    color: hexToRgb(hex),
    opacity
...
function makeRgba(hex: string, alpha: number): RGBA {
...
function createText(text: string, fontSize: number, color: string): TextNode {
...
function applyPngExport(node: ExportMixin): void {
  node.exportSettings = [
    {
      format: "PNG",
...
function roundedPolygonPath(points: Point[], radius: number): string {
  ...
  points.forEach((point, index) => {
    ...
    commands.push(
      `Q ${point.x.toFixed(2)} ${point.y.toFixed(2)} ${end.x.toFixed(2)} ${end.y.toFixed(2)}`
    );
  });
...
function createZMark(scale: number, monochrome = false): GroupNode {
  ...
  const nodes = segments.map((segment) => {
    const vector = createPathShape(
      segment.name,
      segment.points.map((point) => ({ x: point.x * scale, y: point.y * scale })),
      22 * scale,
      makeSolidPaint(accent, monochrome ? 1 : segment.opacity)
    );
...
function addDepthEllipses(parent: FrameNode, size: number): void {
  const ellipses = [
...
function buildIconComposition(
  parent: FrameNode,
  size: number,
  options: { panel?: boolean; monochrome?: boolean; background?: boolean; mark?: boolean } = {}
): void {
  const includeBackground = options.background ?? true;
  const includePanel = options.panel ?? true;
  const includeMark = options.mark ?? true;
...
function cloneIconToSize(size: number): FrameNode {
...
function addLabel(parent: BaseNode & ChildrenMixin, text: string, x: number, y: number, color = TOKENS.textMuted): TextNode {
...
function createSectionLabel(page: PageNode, title: string, x: number, y: number): void {
...
function createPages(): { pages: PageMap; fallback: boolean } {
...
function buildCover(page: PageNode, offsetX: number, offsetY: number): void {
...
function buildMaster(page: PageNode, offsetX: number, offsetY: number): void {
...
function buildIosExports(page: PageNode, offsetX: number, offsetY: number): void {
...
function buildAndroidAdaptive(page: PageNode, offsetX: number, offsetY: number): void {
