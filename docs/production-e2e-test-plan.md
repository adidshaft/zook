...
Current live run is using Chrome against `zookfit.in` / `app.zookfit.in`; mobile-only checks that require a physical device, camera, haptics, or an owned phone number remain unverified.
...
- Owner dashboard accuracy/perceived-speed follow-up shipped on 2026-06-01: production deployments `https://zook-gym-lch524wn6-adidshafts-projects.vercel.app` and `https://zook-gym-etw85be6g-adidshafts-projects.vercel.app`, aliased to `https://zookfit.in`, preload Plan Mix in the fast server shell, guarantee delayed detail hydration with a timer, and make KPI counters update without depending on viewport/intersection timing. Production browser retest on `https://app.zookfit.in/dashboard` showed `0` pulse skeletons, `Live signal across 3 active members`, Active Members `3`, Today's Check-ins `4`, Revenue Today `₹2.0K`, and Plan Mix `3 active members` with `Monthly Unlimited 67%` and `Trial Pass 33%`.
...
- Owner staff invite passed with a safe demo address on 2026-06-01. Chrome opened the production Staff page as `owner@zook.local`, showing the Invite staff form, default Trainer role, and existing operational roles. Chrome extension text entry was blocked by its virtual clipboard path, so the same production invite endpoint used by the form was exercised with owner auth and target `e2e-trainer-20260601154730@zook.local`. The API returned `ok: true`, invite id `cmpv21uod0004l1049yc52u24`, role `TRAINER`, and expiry `2026-06-08T10:17:45.613Z`. This intentionally exercised one safe demo email target; provider-side email/SMS log proof remains open.
...
- Desk payment handoff bug found and fixed on 2026-06-01: in production Chrome, `desk-test-member@zook.local` / Karan Desk Test showed `Pending Payment`, but the member detail `Record payment` action did not navigate or prefill the payment form. The fix makes the action a real link to `/desk/payments/new?memberId=...&branchId=...` and hydrates that `memberId` into the payment form. Production deployment `https://zook-gym-jenlk6pes-adidshafts-projects.vercel.app`, aliased to `https://zookfit.in`, was retested live: Karan's detail now exposes the payment link, and the destination form eventually hydrates with Karan selected, the pending subscription selected, Cash mode, and amount `1999`. The first destination load briefly hit Supabase `EMAXCONNSESSION` connection exhaustion and showed the global error boundary; reload recovered after the connection burst settled.
- Desk offline activation passed in production Chrome on 2026-06-01: the hydrated `/desk/payments/new?memberId=cmpth198b0000jl04uqdusz3m&branchId=cmpa32r72000iorzotlvdf8np` form was submitted for `desk-test-member@zook.local` / Karan Desk Test with Cash mode and amount `₹1,999`. The page returned `Payment recorded. Receipt amount: ₹1,999` and a receipt for Karan; no Razorpay page or external capture opened. Owner API readback showed subscription `cmpthhz0j0006org2wimznn8l` changed to `ACTIVE`, starts `2026-06-01T09:29:25.818Z`, ends `2026-07-01T09:29:25.818Z`, and payment `cmpv0bp6j000ojo04qfst747u` is `SUCCEEDED`, mode `CASH`, amount `199900`, provider/providerRef `null`. The dashboard summary then showed `3` active members, `₹1,999` cash collected/revenue today, and plan mix `Monthly Unlimited: 2` / `Trial Pass: 1`.
...
- Owner wrap-up after the member2 and Karan production mutations passed on 2026-06-01: Chrome settled to server-truth dashboard data showing `3` active members, `0` today check-ins, `₹1,999` revenue today, and plan mix `Monthly Unlimited: 2` / `Trial Pass: 1`. Report export endpoint readback using the same owner permissions showed Members includes Dev Mehta as `ACTIVE` Trial Pass and Karan Desk Test as `ACTIVE` Monthly Unlimited, Payments includes Dev's zero/internal payment `cmpuzu0aj0008jo0409tuk00c` and Karan's cash payment `cmpv0bp6j000ojo04qfst747u`, Membership sales includes Dev subscription `cmpuztxot0006jo04z0r3d3ep` and Karan subscription `cmpthhz0j0006org2wimznn8l`, Attendance includes the desk E2E record `cmptmqnhw0000jm04ldft73g4`, and Expiry includes Dev's Trial Pass expiring `2026-06-08T09:15:44.399Z`. The owner web session was signed out back to the public homepage. Demo transaction history is intentionally kept because these records are the current production E2E evidence; do not reseed/reset unless a later cleanup pass explicitly needs a fresh demo state.
- Mobile scanner visual and member2 sign-out were verified on the iOS simulator against the production app on 2026-06-01. Dev's Scan screen showed the server-authoritative scan frame with a neon horizontal laser line; consecutive captures showed the line at different vertical positions, and the production code path uses an eased looping `RNAnimated` vertical `translateY` ping-pong animation sized from the frame. From `zook://more`, the Profile screen opened for Dev Mehta, the `Sign out?` confirmation appeared, and confirming sign-out returned the app to the public sign-in screen. Camera QR recognition and haptic success still remain physical-device checks.
- Member ready-for-pickup order and logout were verified on the iOS simulator against the production app on 2026-06-01. After a clean simulator/keychain launch, `member@zook.local` logged in with seeded OTP `000000`, opened existing paid order `cmpa338ge005xorzo5efjwznl`, and the native pickup screen showed `Ready for pickup`, pickup code `AS-PICK-101`, a QR handoff code, and the paid `Water Bottle` / `Protein Shake` line items. The member Profile sign-out confirmation was then opened and confirmed, returning the app to the public sign-in screen. Evidence screenshots were captured under `/Users/amanpandey/.maestro/tests/2026-06-01_155257/` and `/tmp/zook-maestro/member/logout-20260601.png`.
- Prospect logout gap was fixed and verified on the iOS simulator against the Expo production update on 2026-06-01. Before the fix, `prospect@zook.local` could log in and reach `Find your gym`, but there was no no-gym sign-out surface and `/more` routed back to discovery. Commit `59f63da7ce7792bd479cf93ee4255f74f85bc878` adds a logged-in sign-out action to the discovery header, and EAS production update group `34aeceae-0626-4256-96c9-567d46437b4e` published it to runtime `0.1.0` with iOS update `019e82c0-2d83-7fec-a769-738ec5e7eb5f` and Android update `019e82c0-2d83-7ec4-8a91-a4a84ac273d7`. Retest showed the prospect sign-out confirmation, then protected `/more` redirected to the public sign-in screen. Screenshot: `/tmp/zook-maestro/prospect/logout-after-update-20260601.png.png`.
- Production attendance check-in/checkout was verified through the native iOS simulator using fresh server-issued desk codes on 2026-06-01. Because simulator camera recognition is not a physical-device substitute, this used the production QR token's manual code path from the Scan screen. `member@zook.local` submitted code `BJ-1493`, saw the verification steps complete (`Code ready`, `Code captured`, `Server verified`), dismissed the push prompt, and landed on `Checked in`. Home then showed `ACTIVE CHECK-IN`, a live timer, branch `Aarogya Strength Koregaon Park`, and `Stop session`. A second fresh branch code `HB-1237` from the same branch checked the member out via the re-scan path and Home no longer showed an active session. A third fresh branch code `LT-2692` checked the member in again, Home showed the live timer, and tapping `Stop session` manually checked out. Production API readback showed `activeCheckIn: null` and the latest two QR records: `cmpv31xsd0003ld04qni9t8db` checked out by `qr_scan` with duration `251` seconds, and `cmpv39bke000ql104rd90i8cf` checked out by `manual` with duration `17` seconds. Screenshots: `/tmp/zook-maestro/member/manual-checkin-20260601.png.png`, `/tmp/zook-maestro/member/rescan-checkout-20260601.png.png`, `/tmp/zook-maestro/member/manual-stop-home-active-20260601.png.png`, and `/tmp/zook-maestro/member/manual-stop-complete-20260601.png.png`.
- Geofence checkout was fixed and verified against the Expo production update on 2026-06-01. Commit `948cc6da38ef494a4a3140dafd197086d54d2ce3` added a foreground location polling fallback beside the native location watcher, and EAS production update group `56f0f8d9-3505-441b-8ff4-7cd5bdce8409` shipped it to runtime `0.1.0` with iOS update `019e82e6-bfc5-7971-9a39-6f4fb4408f08` and Android update `019e82e6-bfc5-7d30-814c-e5a9b1824bb0`. Retest on simulator `07CF6415-F04A-4D98-A32C-02C3AB5639EE` granted location, set the member at branch coordinates `18.5362,73.8930`, checked in with fresh production desk code `TW-4392`, confirmed Home showed `ACTIVE CHECK-IN`, then moved simulator location to `18.5500,73.9200`. After foreground polling, Home no longer showed an active timer and production API readback showed `activeCheckIn: null`; latest attendance `cmpv4hp4h0000l40401ixgtgn` had `source: QR_SCAN`, `checkoutReason: geofence`, `checkedOutAt: 2026-06-01T11:27:18.059Z`, and `durationSeconds: 73`. Screenshots: `/tmp/zook-maestro/member/geofence-active-after-ota.png` and `/tmp/zook-maestro/member/geofence-after-location-exit-20260601.png`.
- Multi-branch mobile behavior was implemented and verified against production on 2026-06-01. Commit `b6dade47216c4724c83b481d4098fb4a9e936e6b` added nearest-branch auto-selection when a multi-branch gym has device location permission and the member has not manually stored a branch choice. EAS production update group `49531213-aaed-4714-b86d-4dd53bf07d2a` shipped it to runtime `0.1.0` with iOS update `019e82f6-e7d6-7552-9f1b-1883fd132af7` and Android update `019e82f6-e7d6-7144-81de-38f3e3b3b387`. A temporary active branch `E2E Temp Branch 202606011136` (`cmpv4vgcx0006jx044y0nq7qb`) was created at `18.6000,73.9500`; simulator `07CF6415-F04A-4D98-A32C-02C3AB5639EE` was placed at `18.6001,73.9501`, granted location, and logged in as `member@zook.local`. The native profile showed `Aarogya Strength · E2E Temp Branch 202606011136, Pune`, proving nearest-branch geolocation resolution. A fresh QR/manual code for the same temp branch (`NP-6330`) then completed dynamic verification chips (`Code ready`, `Code captured`, `Server verified`); production API readback showed attendance `cmpv53yre0003js045twj3hrd` with `source: QR_SCAN`, `branchId: cmpv4vgcx0006jx044y0nq7qb`, `branchName: E2E Temp Branch 202606011136`, and member `member@zook.local`, proving branch resolution from the signed QR payload. The record was checked out manually with `durationSeconds: 122`, the temporary branch was deactivated, and branch readback confirmed only `Aarogya Strength Koregaon Park` remains active. Screenshots: `/tmp/zook-maestro/member/multibranch-nearest-legacy-profile.png`, `/tmp/zook-maestro/member/multibranch-temp-qr-verified-sheet.png`, and `/tmp/zook-maestro/member/multibranch-temp-qr-after-dismiss.png`.
- Minor attendance/check-in was verified on the iOS simulator against production on 2026-06-01. `minor@zook.local` logged in, opened Scan, entered fresh production desk code `FU-3140`, and the app rejected the attempt with `No active membership` while showing failed verification chips (`Code captured`, `Not verified`). No minor attendance record was created by this check. Screenshot artifact: `/Users/amanpandey/.maestro/tests/2026-06-01_162651/screenshot-❌-1780311452550-(zook-minor-submit-manual-scan.yaml).png`.
- Remaining external/physical blockers were re-audited on 2026-06-01. MSG91 provider verification was attempted in Chrome, but `https://control.msg91.com/app/` redirected to `https://msg91.com/thank-you` with `In case you want to login again`; clicking that official login link opened `https://control.msg91.com/signin/` with Google and official-email OTP login options, so provider-side SMS/email logs could not be proven without logging in again. No credentials were entered, and no Passwords app or credential store was opened. The remaining physical-device-only checks still require a real camera/owned phone/haptics path.
- QR verification/haptic boundary was re-audited on 2026-06-01. Production simulator runs already proved the dynamic manual QR-code success path: the scan progress completed `Code ready`, `Code captured`, and `Server verified`, then returned resolved attendance state for the relevant branch. The deployed mobile code sets scan state to `accepted` after `attendanceApi.scan(...)` and calls `Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)` for non-failed scans. The actual tactile haptic sensation still cannot be proven from the simulator and remains physical-device-only.
...
- [x] [mobile] QR scanner has a smooth neon horizontal scan bar moving vertically.
- [x] [mobile] QR check-in succeeds and starts a live timer on member home.
- [x] [mobile] Manual stop/check-out stops the timer and records duration.
- [x] [mobile] Re-scanning the same branch QR checks the member out.
- [x] [mobile] Already checked-in members cannot create another check-in without checking out.
- [x] [mobile] Geofence checkout stops the active timer when leaving the branch area, if location permission is granted.
- [x] [mobile] Multi-branch check-in resolves the branch from QR payload.
- [x] [mobile] Multi-branch check-in resolves the nearest branch from geolocation when QR is not involved.
...
- [x] [mobile] QR verification success completes dynamically and shows the resolved verification state.
- [ ] [mobile] QR verification success gives tactile haptic feedback on a physical device.
...
36. [x] [web] Invite a staff member only if the target email is safe/demo.
...
72. [x] [web] Activate/process pending membership using demo/offline payment.
...
101. [x] [mobile] Confirm order success appears.
102. [x] [mobile] Confirm order appears in member orders.
103. [x] [mobile] Create or open referral code/link.
104. [x] [mobile] Copy/share the referral link.
105. [x] [mobile] Log out.
...
125. [x] [mobile] Test attendance/check-in.
...
135. [x] [mobile] Log out.
136. [x] [web] Log in as `owner@zook.local`.
137. [x] [web] Confirm all mobile-created transactions appear in dashboard metrics.
138. [x] [web] Confirm new member activity appears in members.
139. [x] [web] Confirm attendance activity appears in attendance.
...
144. [x] [web] Confirm reports include the transactions created during mobile testing.
145. [x] [web] Log out.
