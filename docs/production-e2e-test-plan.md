...
- Platform dashboard loading/formatting follow-up prepared on 2026-06-01: `/platform` now streams the authenticated shell, grouped navigation, and compact first-fold structure before the platform read model resolves; the metric cards use a smaller platform-specific numeric scale, and the previous full-page wait moves behind a section-level skeleton. Local `@zook/web` lint, build, and post-build typecheck passed. Production deployment and Chrome retest are next.
