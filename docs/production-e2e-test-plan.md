...
- Remaining external/physical blockers were re-audited on 2026-06-01. MSG91 provider verification was attempted in Chrome, but `https://control.msg91.com/app/` redirected to `https://msg91.com/thank-you` with `In case you want to login again`, so provider-side SMS/email logs could not be proven without logging in again. No Passwords app or credential store was opened. Production branch readback for `Aarogya Strength` returned exactly one active branch (`Aarogya Strength Koregaon Park`, id `cmpa32r72000iorzotlvdf8np`), so multi-branch QR/geolocation behavior cannot be verified against the current production demo state without intentionally creating extra branch/subscription data. The remaining physical-device-only checks still require a real camera/owned phone/haptics path.
