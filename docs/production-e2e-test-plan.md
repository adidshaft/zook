...
- Platform dashboard follow-up fix prepared on 2026-05-31: inactive platform sections are no longer mounted behind `hidden` classes, Status and Incidents render separately, the sticky platform nav/header are denser, forced anchor scrolling was removed, and heavy platform org/subscription queries now select/page only the rows needed for the current console view.
