...
- [ ] iOS TestFlight build available to tester after Apple Beta App Review.
...
- App Store Connect direct verification passed after Apple login: build `0.1.0 (5)` is visible, upload status is `Complete`, and status is `Ready to Submit`.
- Tester `aman0902pandey@gmail.com` was added as the only individual tester and build `0.1.0 (5)` was submitted for Apple Beta App Review. App Store Connect showed `1 tester has been added`, `Remove from Review`, and tester status `No Builds Available`, so TestFlight install availability is pending Apple review/processing.
